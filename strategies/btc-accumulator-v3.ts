 ...
 * Comme l'équité est en BTC, l'excursion vente→rachat ne porte que sur la part
 * vendue → gains ET pertes ~divisés par sellFraction (échelle proportionnelle).
 * sellFraction=1 ⇒ comportement identique à la v2 (contrôle de parité).
 *
 * Implémentation : on ne peut plus déduire l'état « vendu » de position.qty
 * (on détient encore (1-f) BTC pendant l'excursion) → flag d'état explicite
 * `ctx.state['sold']`.
 *
 * --- ci-dessous, doc héritée de la v2 ---
...
export default defineStrategy({
  name: 'BTC Accumulator v3',
  description:
    "Comme la v2 (tendance 3d + double confirmation) mais avec VENTE FRACTIONNÉE : ne vend qu'une part du BTC en bear (sellFraction) pour baisser le drawdown. Spot, dénomination BASE.",
  markets: ['spot'],
  ...
  params: {
    ...
    sellFraction: p.number({
      default: 0.5,
      min: 0.1,
      max: 1,
      step: 0.05,
      label: 'Fraction du BTC vendue en bear',
      description:
        "Part du BTC vendue à chaque excursion baissière. 1 = tout (= comme la v2). Plus bas = drawdown plus faible MAIS gains plus faibles (échelle ~proportionnelle). 0,5 = on coupe le risque de moitié.",
      group: 'Risque',
    }),
  ...
  async onCandle(ctx, feedId, candle) {
    ...
    const e = ema.value!
    const a = atr.value ?? 0
    // état explicite : on détient encore (1-f) BTC pendant l'excursion, donc on
    // ne peut PAS déduire « vendu » de position.qty → flag dédié.
    ...
    if (!sold) {
      ...
      if (bearRegime && cleanTrend && sellingFlow && belowEma && a > 0) {
        // vente FRACTIONNÉE : seulement une part du BTC détenu
        const qty = ctx.roundQty(ctx.position.qty * ctx.params.sellFraction)
        if (qty <= 0) return
        ...
        await ctx.order.market({
          side: 'SELL',
          qty,
          reason: `Régime baissier (tendance ${ctx.params.trendInterval}${ctx.params.useConfirm ? `+${ctx.params.confirmInterval}` : ''} en déclin, ER ${er.value!.toFixed(2)}, flow ${f.toFixed(2)}, prix < EMA${ctx.params.emaLen}) → vente de ${(ctx.params.sellFraction * 100).toFixed(0)}% du BTC pour rachat plus bas`,
          tag: 'entry',
        })
      }
      return
    ...
    // ---- excursion en cours (on détient l'USDT de la vente fractionnée) :
    //      racheter quand la baisse s'essouffle
    if (candle.close > rebuyEma.value!) {
      ...
      if (usdt <= 0) {
        ctx.state['sold'] = false // plus rien à racheter → excursion close
        return
      }
      const soldPx = (ctx.state['soldPrice'] as number | undefined) ?? candle.close
      const gainPct = ((soldPx - candle.close) / soldPx) * 100
      await ctx.order.market({
        side: 'BUY',
        quoteQty: usdt,
        reason: `Rachat sur recroisement EMA${ctx.params.rebuyEmaLen} (vendu ${soldPx.toFixed(0)} → rachat ${candle.close.toFixed(0)}, ${gainPct >= 0 ? '+' : ''}${gainPct.toFixed(1)}% en BTC)`,
        tag: 'exit',
      })
    }
  },
  ...
  async onFill(ctx, _fill, order) {
    // pose du stop après la vente fractionnée. NE dépend PLUS de position.qty
    // (en fractionné on détient encore du BTC) → on se base sur le tag + bracket.
    ...
    // rachat (recroisement ou stop) exécuté → excursion close
    if (order.tag === 'sl' || order.tag === 'exit') {
      ctx.state['bracket'] = false
  ...
  async onStop(ctx) {
    await ctx.order.cancelAll()
    // on veut TOUJOURS finir en détenant 100% en BTC : si une excursion est
    // ouverte (USDT en main), on rachète.
