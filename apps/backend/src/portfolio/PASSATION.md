...
11. **Telegram fire-and-forget : deux bugs en cascade** (vécu 2026-07-19/20).
    (a) `sendTelegram` non attendu + `process.exit` → l'envoi meurt avec le
    process (résumé nocturne parfait, jamais reçu — course gagnée nuit 1,
    perdue nuit 2). (b) Une fois (a) corrigé par un simple flush des envois
    en vol, la capture Telegram de Mario a montré le vrai fond : 5 envois
    CONCURRENTS vers le même chat → se doublent en route (ordre mélangé) et
    déclenchent un 429 avalé par le `.catch` → l'en-tête regime1 (le message
    le plus utile) sautait. Correctif FINAL : `services/telegram.ts` est
    désormais une FILE sérialisée — `sendTelegram` empile (non bloquant,
    vital pour le trading), un worker unique draine dans l'ordre, espacé
    350 ms, retry sur 429 (honore retry_after). `flushTelegram(maxMs)`
    attend la file (borné) avant tout `process.exit` : tick.ts (15 s, pas de
    deadline), shutdown backend (4 s, docker SIGKILL ~10 s). Tout NOUVEAU
    script court qui notifie doit flusher avant de sortir. Au passage : même
    une mort BRUTALE du VPS (reset hyperviseur 2026-07-18 16:34 UTC, journal
