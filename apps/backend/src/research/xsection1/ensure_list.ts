/** Télécharge le 1d spot 2019-01→now pour chaque symbole du fichier
 *  $SYMBOLS_FILE (un par ligne). Erreurs tolérées (symbole ignoré). */
...
for (const symbol of syms) {
  try {
    await store.ensureRange('spot', symbol, '1d', START, END, {})
  } catch (err) {
...
console.log(`univers 1d terminé (${done} symboles)`)
process.exit(0)
