...
| H9 | **Événementiel** : listings/délistings, halvings, unlocks, inclusions | flux d'offre/demande datés et publics | dates dérivées des candles Vision (fiables) ; unlocks = source externe à trouver ; halvings n=4 | **listing1 🎯 SIGNAL VALIDÉ IS+OOS (2026-07-16)** : drift post-listing −22 %/30 j IS (5/5 années nég., percentile 0,0) → OOS −26 % PLUS FORT, tradabilité 18 %→93 % (perps quasi simultanés désormais), funding short méd +0,4 % ; placebo « âge chronique » expliqué ; NEXT = protocole stratégie short-new-listings EN CHEMIN quotidien (path risk des pumps = le vrai juge) ; délistings documentaire −16 %/7 j ; halvings/unlocks vierges | 🎯 signal → protocole strat |
