...
Plateforme de trading algorithmique crypto **Spot + Futures USDT-M** : **exécution sur OKX** (compte, ordres, fills, levier/marge), **données de marché Binance** (archive publique Vision + flux WS publics). Bots live, paper trading, backtesting haute fidélité, optimisation de stratégies (grid + walk-forward), UI web temps réel. Conçue pour tourner sur un VPS (UI protégée par mot de passe), avec les backtests en local sur une grosse machine.
...
- **IP géo-bloquée par Binance (erreur 451)** : les données spot basculent automatiquement sur le miroir officiel `data-api.binance.vision`, le funding passe par les archives Vision — backtests et recherche fonctionnent. Les données futures récentes nécessitent une IP autorisée (votre VPS) ; l'exécution passe par **OKX** et n'est pas concernée.
- **Clés API OKX** : stockées chiffrées (AES-256-GCM avec `MASTER_KEY`). Créez des clés de trading sans droit de retrait, avec whitelist IP.
- **Un bot futures par paire et par mode** (live/testnet) : OKX fusionne les positions d'un même symbole sur le compte.
